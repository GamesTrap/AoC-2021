...
int32_t main()
{
    ...
    //Logic
    int32_t allFlashStep = 0;
    int32_t gridSum = -1;
    while(gridSum != 0) //Stop if all flashed
    {
        gridSum = 0;
        ++allFlashStep;
        ...
        //Count flashes (when all flash gridSum should be 0)
        for(int32_t y = 0; y < grid.size(); ++y)
        {
            for(int32_t x = 0; x < grid[0].size(); ++x)
            {
    ...
    std::cout << "First step where all octopuses flash: " << allFlashStep << std::endl;
